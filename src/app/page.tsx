...
export default function Home() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (isVideoPlay == 1) {
        ...
        setTimeout(() => {
          setAnimatedClass('opacity-1 bottom-[-3%]')
        }, 35000);
        setVideoPlay(1);
        playVideo();
      }
  ...
  return (
    <div className='bg-[#0E1B37] relative min-h-screen overflow-scroll w-full'>
        ...
        <div className="hidden md:hidden lg:flex flex-col z-1 relative w-full section-2 justify-center items-center">
        ...
        <div className="lg:hidden flex h-[100vh] w-full justify-center items-center relative">
          <div className="flex h-[50vh] relative justify-center items-center flex-col mt-10 w-full">
            <video muted={true} id="v1" tabIndex={0} preload="preload" className='w-full flex justify-center items-center z-0 relative' controls={false}>
              <source type="video/mp4" src={"/midaz_video_mobile.mp4"}></source>
            </video>
            <span className={`flex flex-col lg:hidden absolute translate-x-[-50%] text-[1rem] z-20 left-[50%] transition-all w-[90%] text-center ${animatedClass} ${isVideoPlay}`}>For reservations, please reach us via <Link className='text-golden' href="/pages/contact" as={'/pages/contact'} style={{transitionDuration : '1s'}}>Contact Page</Link></span>
